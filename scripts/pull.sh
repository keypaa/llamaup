...
# ---------------------------------------------------------------------------
# install_binary — extract tarball and create wrappers for all binaries found
# Args:
#   $1 = archive_path
#   $2 = install_dir
# ---------------------------------------------------------------------------
install_binary() {
  ...
  # Parent bin dir — users add this one directory to PATH
  local bin_dir
  ...
  # Create a wrapper for every executable in bin/ (not just the hardcoded 3)
  local wrapped=0
...
EOF
    ...
    (( wrapped++ )) || true
  done < <(find "${install_dir}/bin" -maxdepth 1 -type f -executable 2>/dev/null)

  success "Installed ${wrapped} wrapper(s) in ${bin_dir}"
}
...
main() {
  ...
  if [[ "$dry_run" == "true" && "$do_list" == "false" ]]; then
    ...
    echo -e "    6. Create wrappers for all binaries in bin/"
  ...
  echo
  echo -e "  ${BOLD}Key tools (all others also available in PATH):${RESET}"
  echo -e "    llama-cli      # Command-line inference"
  echo -e "    llama-server   # HTTP API + web UI"
  echo -e "    llama-bench    # Benchmark performance"
  echo -e "    llama-quantize # Re-quantize GGUF models"
  echo -e "    llama-embedding # Generate embeddings"
